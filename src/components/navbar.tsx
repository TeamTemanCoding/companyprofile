...
export default function Nav() {
  ...
  return (
    <div className="bg-black/5 backdrop-blur-md w-screen fixed z-50 flex justify-between items-center px-4 md:px-8">
      {/* Logo */}
      <Image
        width={80}
        height={80}
      ...
      >
        ...
        {navLinks.map(({ label, href }) => (
          <button
            key={label}
            data-label={label}
            className={`relative z-10 px-5 text-sm py-1 ${
              active === label ? "text-white " : "hover:text-gray-300"
            }`}
      ...
      {isMobileMenuOpen && (
        ...
        >
          {navLinks.map(({ label, href }) => (
            <button
              ...
              className={`text-sm font-bold py-2 px-4 rounded-md ${
                active === label ? "bg-purple-600" : "hover:bg-purple-700"
              }`}
              onClick={() => {
                handleClick(href);
          ...
          <a href="https://linktr.ee/temancoding.com" className="bg-purple-700 w-full justify-center items-center h-10 rounded-full text-white mr-8 hover:bg-purple-900 hover:scale-110 transition-all duration-200 ease-out flex">
            Order
          </a>
        </motion.div>
      )}
      ...
      <div className="hidden md:block">
        <a href="https://linktr.ee/temancoding.com" target="_blank"  className="bg-purple-700 px-10 py-2 rounded-full text-white hover:bg-purple-900 hover:scale-110 transition-all duration-200 ease-out">
          Order
        </a>
      </div>
    </div>
